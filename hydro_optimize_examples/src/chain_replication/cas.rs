...
impl<'a, 'b, State, RequestId, Sender> CASLike<'a, State, RequestId, Sender> for MockCAS<'a, 'b>
where
    ...
    fn build(
        ...
        let (state_updates, read_outputs) = sliced! {
            ...
            let read_outputs = reads
                .cross_singleton(curr_state.clone());

            (curr_state.into_stream(), read_outputs)
        };

        let write_outputs = atomic_writes.end_atomic().keys();
        let read_outputs = read_outputs.map(q!(|((sender, req_id), (_writer_id, state))| (
            sender,
            (req_id, state)
        )));
